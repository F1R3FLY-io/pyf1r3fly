...
from f1r3fly.certificate import (
...
def test_get_node_tls_key_pem(private_key_pem: bytes, tls_cn_name: str, expected_tls_cn: str) -> None:
    ...
    output_pem = get_node_tls_key_pem(private_key)
    # Verify output is valid PEM and contains the same key
    reloaded_key = load_pem_private_key(output_pem.encode('utf8'), None, default_backend())
    # Compare the key numbers to ensure they are equivalent
